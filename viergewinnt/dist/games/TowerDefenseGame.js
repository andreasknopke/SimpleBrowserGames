export class TowerDefenseGame {
    ...
    setupEventListeners() {
        ...
        if (this.resetTdBtn)
            this.resetTdBtn.addEventListener('click', () => this.init());
        if (this.tdNextWaveBtn)
            this.tdNextWaveBtn.addEventListener('click', () => this.startNextWave());
    }
    init() {
        if (this.tdInterval)
            clearInterval(this.tdInterval);
        if (!this.tdCanvas || !this.tdCtx)
            return;
        this.tdGold = 180;
        this.tdLives = 20;
        this.tdScore = 0;
        ...
        if (proj.type === 'fireball' && proj.splash) {
            for (const e of this.tdEnemies) {
                ...
                if (dist <= proj.splash) {
                    ...
                    if (e !== enemy)
                        this.spawnParticles(e.x, e.y, '#ff6600', 3);
                }
            }
    ...
    handleTdClick(e) {
        if (this.tdGameOver || this.tdWon || !this.tdCanvas)
            return;
    ...
    startNextWave() {
        if (this.tdWaveActive || this.tdGameOver || this.tdWon)
            return;
        if (this.tdWave < this.TD_WAVES.length) {
            this.tdWave++;
    ...
    updateUi() {
        if (this.tdGoldDisplay)
            this.tdGoldDisplay.textContent = String(this.tdGold);
        if (this.tdLivesDisplay)
            this.tdLivesDisplay.textContent = String(this.tdLives);
        if (this.tdWaveDisplay)
            this.tdWaveDisplay.textContent = `${this.tdWave}/${this.TD_WAVES.length}`;
        if (this.tdScoreDisplay)
            this.tdScoreDisplay.textContent = String(this.tdScore);
        if (this.tdStatus) {
            if (this.tdWon) {
...
//# sourceMappingURL=TowerDefenseGame.js.map