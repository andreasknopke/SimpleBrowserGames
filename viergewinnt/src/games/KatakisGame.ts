...
type EnemyType = 'fighter' | 'bomber' | 'turret' | 'runner' | 'boss';
...
type PowerUpType = 'laser' | 'spread' | 'force' | 'speed' | 'health';
...
interface ForceModule {
    x: number;
    y: number;
    active: boolean;
    attached: boolean;
    angle: number;
    side: number; // 1 = right/front, -1 = left/back
}
...
export class KatakisGame implements Game {
    public readonly id: GameId = 'katakis';

    private readonly CANVAS_W = 640;
    private readonly CANVAS_H = 400;
    private readonly PLAYER_W = 36;
    private readonly PLAYER_H = 28;

    // Core state
    private gameInterval: ReturnType<typeof setInterval> | undefined;
    ...
    private pHp = 5;
    private pMaxHp = 5;
    private pInvincible = 0;
    private pWeapon: 'basic' | 'laser' | 'spread' = 'basic';
    private pWeaponLevel = 1;
    private pScore = 0;
    private pLives = 3;
    private pShootTimer = 0;
    private pShootInterval = 12;
    ...
    private force: ForceModule;

    // Level
    private level = 0;
    private levelTimer = 0;
    private levelEnemyTimer = 0;
    private scrollX = 0;
    ...
    private btnUp: HTMLElement | null = null;
    private btnDown: HTMLElement | null = null;
    private btnShoot: HTMLElement | null = null;
    private btnForce: HTMLElement | null = null;

    constructor() {
        this.canvas = document.getElementById('katakisCanvas') as HTMLCanvasElement | null;
    ...
    private setupMobileControls(): void {
        const addCtrl = (btn: HTMLElement | null, action: () => void) => {
            if (!btn) return;
        ...
        addCtrl(this.btnLeft, () => { if (!this.gameOver) this.px = Math.max(10, this.px - 12); });
        addCtrl(this.btnRight, () => { if (!this.gameOver) this.px = Math.min(this.CANVAS_W - this.PLAYER_W - 10, this.px + 12); });
        addCtrl(this.btnUp, () => { if (!this.gameOver) this.py = Math.max(10, this.py - 12); });
        addCtrl(this.btnDown, () => { if (!this.gameOver) this.py = Math.min(this.CANVAS_H - this.PLAYER_H - 10, this.py + 12); });
        addCtrl(this.btnShoot, () => { if (!this.gameOver) this.playerShoot(); });
        addCtrl(this.btnForce, () => { if (!this.gameOver) this.toggleForce(); });
    }
    ...
    private updateLevel(): void {
        ...
        const spawnEnemy = (type: EnemyType, x: number, y: number, hp: number) => {
        ...
        // Level 4 - Final boss
            ...
            if (this.bossActive && this.enemies.filter(e => e.type === 'boss').length === 0 && this.levelTimer > 1100) {
                this.gameOver = true;
                this.gameWon = true;
                if (this.statusEl) this.statusEl.textContent = `🎉 KATAKIS BESIEGT! Punkte: ${this.pScore}`;
            }
        }
    }
    ...
    private spawnPowerUpAt(x: number, y: number): void {
        const types: PowerUpType[] = ['laser', 'spread', 'force', 'speed', 'health'];
    ...
    private updateStatus(): void {
        ...
        const weaponNames: Record<string, string> = { basic: 'Basic', laser: 'Laser', spread: 'Spread' };
